...
print(patho.head())
non_patho = pd.read_csv('./Domains/genes_with_pfam.txt', sep='\t')
...
non_patho_genes = non_patho.groupby('PFAM')['Gene(s)'].apply(lambda x: ';'.join(x)).to_dict()
...
def get_non_patho_genes(pfam):
    ...
    matching_genes = non_patho[mask]['Gene(s)']
